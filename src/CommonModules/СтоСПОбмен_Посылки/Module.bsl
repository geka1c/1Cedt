...
Процедура Загрузить_ПосылкиПоДате(ПоследняяДатаЗагрузки) Экспорт
	...
	Если АвторизацияВыполнена тогда
		Константы.ПоследняяЗагрузкаПосылок.Установить(ТекущаяДата());
	КонецЕсли;	
КонецПроцедуры
...
Функция Загрузить_СоставПосылкиПоКодам(СписокКодовПараметр) Экспорт
	...
	Иначе	
		возврат Разбор_СоставПосылкиПоКодам(ТЗ,хмл_получили);
	КонецЕсли;
