...
Функция РассчитатьСтоимость(Параметры) Экспорт
	...
	Если СтруктураРасчета.Успешно  и СтруктураРасчета.Свойство("totalrate") Тогда
		Протокол.Результат	  =	"ok";
		Протокол.ТарифТК				= СтруктураРасчета.totalrate/100;
	...
	Иначе
		...
		Протокол.РасчетКалькулятора		= СтруктураРасчета.ПолученныеДанные;
...
Функция РасчетСтоимости(Параметры) Экспорт
	...
	Если Параметры.Негаборит Тогда											
...
Функция СформироватьЗаказ(Параметры,ЭтоEMS=ложь) Экспорт
	...
	Данные.Вставить("mail-category", 		ПолучитьЗначениеПеречисления(Параметры["mail-category"])); 		//Категория РПО
	Данные.Вставить("mail-type", 			ПолучитьЗначениеПеречисления(Параметры["mail-type"])); 			//Вид РПО
...
Функция ОбъектJSONВструктуру(ч_JSON)
	стрОтвета=Новый  Структура;	
