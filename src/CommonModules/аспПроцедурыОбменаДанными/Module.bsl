...
Функция 	ПолучитьССайта() Экспорт
	ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Получаем с сайта: Покупки, Участников, Карты участников",,0,100);
	параметрыДаты = новый Структура;
	параметрыДаты.Вставить("ДатаОбъединеннойДоставки",	 Константы.ПоследняяЗагрузкаГруппОбъединеннойДоставки.Получить());			
	параметрыДаты.Вставить("ДатаУчастники",				 Константы.ПоследняяЗагрузкаУчастников.Получить());			
	параметрыДаты.Вставить("ДатаПокупки",				 Константы.ПоследняяЗагрузкаПокупок.Получить());			
	параметрыДаты.Вставить("ДатаКарты",					 Константы.ПоследняяЗагрузкаКарты.Получить());			
	...
	Если АвторизацияВыполнена(ИмяФайлаОтвета) Тогда
		...
		ОбработатьЗагруженныхУчастников(тзУчастники);
		Константы.ПоследняяЗагрузкаУчастников.Установить(ТекущаяДата());
		
		ОбработатьЗагруженныеПокупки(тзПокупки);
		Константы.ПоследняяЗагрузкаПокупок.Установить(ТекущаяДата());

		ОбработатьЗагруженныеКарты(тзКарты);
		Константы.ПоследняяЗагрузкаКарты.Установить(ТекущаяДата());
		
	КонецЕсли;
	
	НачалоЗагрузкиПосылок = Константы.ПоследняяЗагрузкаПосылок.Получить();
...
Процедура 	ПолучитьУчастниковПоДате(ДатаЗагрузки=неопределено) Экспорт
	...
	Если ДатаЗагрузки=неопределено Тогда
		Параметры.Вставить("ДатаУчастники"	, Константы.ПоследняяЗагрузкаУчастников.Получить());
	иначе	
...
Процедура 	ПолучитьУчастниковПоКодам(ПараметрУчастники) Экспорт
	...
	Параметры=новый Структура;
	Параметры.Вставить("token",	Константы.Токен.Получить());
	Параметры.Вставить("xml", 	ПолучитьСтрокуПоКодамСправочника(списокУчастников,"Участники"));
...
Функция 	ПолучитьПокупкиПоДате(ДатаЗагрузки=неопределено) Экспорт
	Параметры = новый Структура;
	Если ДатаЗагрузки=неопределено Тогда
		Параметры.Вставить("ДатаПокупки"	, Константы.ПоследняяЗагрузкаУчастников.Получить());
	иначе	
...
Процедура ОбработатьЗагруженныхУчастников(тзИзФайла) 
	Если не ЗначениеЗаполнено(тзИзФайла) или тзИзФайла.количество()=0 Тогда возврат КонецЕсли;
	Запрос = Новый Запрос;
	...
	Запрос.Текст = 
		...
		|	тзФ.rating 		как Рэйтинг,
		|	тзФ.status 		как Статус,
	...
	Запрос.Текст=
		...
		|					ИЛИ Ф.Рэйтинг - Ф.Ссылка.Рэйтинг > 1
		|					ИЛИ Ф.Рэйтинг - Ф.Ссылка.Рэйтинг < -1
		|					ИЛИ Ф.Статус 	<> 	Ф.Ссылка.Статус))";
	...
	Пока Выборка.Следующий() Цикл
		...
		СП_РаботаСоСправочниками.ОбновитьУчастника(Участник_Ссылка,	параметрыУчастника);
		
		
		Если 	Выборка.Статус=5 или 
				Выборка.Статус=11 или 
				Выборка.Статус=12 или 
			   (ЗначениеЗаполнено(Выборка.СсылкаОрг) и не ЗначениеЗаполнено(Выборка.СсылкаОрг.Участник)) Тогда
			   
			Организатор_Ссылка = 	СП_РаботаСоСправочниками.ПолучитьОрганизатораПо_Коду(КодУчастника);
			СП_РаботаСоСправочниками.ОбновитьОрганизатора(Организатор_Ссылка, параметрыУчастника);
		КонецЕсли;

		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Загрузка Участников: " + счетчик + " из " + ВсегоЭлементов ,,строка(счетчик),строка(ВсегоЭлементов));
	КонецЦикла;
КонецПроцедуры	

Процедура ОбработатьЗагруженныеПокупки(тзИзФайла)  
	Если не ЗначениеЗаполнено(тзИзФайла) или тзИзФайла.количество()=0 Тогда возврат КонецЕсли;
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц=новый МенеджерВременныхТаблиц;
	Запрос.Текст = 
	...
	Пока Выборка.Следующий() Цикл
		...
		параметрыПокупки.Вставить("ГородСП",			ДобавитьВыбратьГородСП(Выборка.КодГорода,Выборка.СсылкаГород));
...
Процедура ОбработатьЗагруженныеКарты(тзИзФайла)  
	Если не ЗначениеЗаполнено(тзИзФайла) или тзИзФайла.количество()=0 Тогда возврат КонецЕсли;
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц=новый МенеджерВременныхТаблиц;
	Запрос.Текст = 
	...
	Пока Выборка.Следующий() Цикл
		счетчик = счетчик + 1;
		ДобавитьКартуИзСтрТЗ(Выборка);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Загрузка Карт участников: " + счетчик + " из " + ВсегоЭлементов ,,строка(счетчик),строка(ВсегоЭлементов));
	КонецЦикла;          
КонецПроцедуры	                 
...
Функция АвторизацияВыполнена(ИмяФайлаОтвета)   Экспорт
	...
	Пока ЧтениеХМЛ.Прочитать() Цикл
		Если ЧтениеХМЛ.Имя="auth" и ЧтениеХМЛ.ТипУзла= ТипУзлаXML.НачалоЭлемента Тогда	
			ЧтениеХМЛ.Прочитать();ЧтениеХМЛ.Прочитать();
			Если ЧтениеХМЛ.Значение="ok" Тогда
				Возврат Истина;
			Иначе
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Не выполнене авторизация при подключении к сайту 100 сп!" );
				Возврат Ложь;
			КонецЕсли;	
		КонецЕсли;	
	КонецЦикла;
...
Функция ПолучитьСтрокуСДатамиЗагрузки(Параметры) 
	...
	Если Параметры.Свойство("ДатаУчастники")			Тогда ДобавитьЭлементXML(ЗаписьXML,"users",			Параметры.ДатаУчастники);  				КонецЕсли;
	Если Параметры.Свойство("ДатаПокупки")				Тогда ДобавитьЭлементXML(ЗаписьXML,"purchases",		Параметры.ДатаПокупки);  				КонецЕсли;
	Если Параметры.Свойство("ДатаКарты")				Тогда ДобавитьЭлементXML(ЗаписьXML,"cards",			Параметры.ДатаКарты);  					КонецЕсли;
	Если Параметры.Свойство("ДатаОбъединеннойДоставки")	Тогда ДобавитьЭлементXML(ЗаписьXML,"deliveryGroups",Параметры.ДатаОбъединеннойДоставки);  	КонецЕсли;
	...
	стр="<?xml version=""1.0"" encoding=""UTF-8""?> "+СтрЗаменить(стр,"xmlns=""http://www.100sp.ru/XMLSchema-instance"" ","");
	Возврат стр;
КонецФункции
...
Функция ПолучитьКарты(ДатаЗагрузки=неопределено) Экспорт
	Параметры	= новый Структура;
	Параметры.Вставить("token",	Константы.Токен.Получить());
	Параметры.Вставить("xml", 	ПолучитьСтрокуСДатамиЗагрузки(новый Структура("ДатаКарты",Константы.ПоследняяЗагрузкаКарты.Получить())));
...
Функция ПолучитьТочкиРаздачи(ДатаЗагрузки=неопределено) Экспорт
	...
	Если АвторизацияВыполнена(ИмяФайлаОтвета) Тогда
		...
		Для каждого точка из тзТочкиРаздачи Цикл
			Если точка.code="" Тогда продолжить; Конецесли;
			...
			параметрыПВ.Вставить("ГородСП", 	 ДобавитьВыбратьГородСП(точка.city_id));
		...
		Для каждого пв из тзПостомат Цикл
			Если пв.unique_id="" Тогда продолжить; Конецесли;
			код=пв.unique_id;
...
Функция ПолучитьГородаСП(СписокГородов=неопределено) Экспорт
	...
	Если АвторизацияВыполнена(ИмяФайлаОтвета) Тогда
		...
		Для каждого точка из тзГорода Цикл
			Если точка.region_id="" Тогда продолжить; Конецесли;
			Если точка.id="" Тогда продолжить; Конецесли;
			...
			Попытка
				обРегион.Записать();
			Исключение
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Не удалось заисать регион с кодом "+точка.region_id);
				Продолжить;
			КонецПопытки;
...
Процедура ПолучитьЗапросомКарты(ПараметрКарты) Экспорт
	Если 	ТипЗнч(ПараметрКарты) = Тип("Строка") или
			ТипЗнч(ПараметрКарты) = Тип("Число")		 Тогда
	...
	ОбработатьЗагруженныеКарты(тзИзФайла);
КонецПроцедуры	
...
Процедура ДогрузитьСправочникиССайта() экспорт
	...
	ОбработатьЗагруженныхУчастников(тзИзФайла);
	#КонецОбласти
...
Процедура ДогрузитьЭлементСправочникаССайтаВФоне(Элемент) экспорт
	...
	ФоновыеЗадания.Выполнить("аспПроцедурыОбменаДанными.ДогрузитьЭлементСправочникаССайта",ПараметрыФЗ,новый УникальныйИдентификатор,"Догружаем элемент справочника с сайта");
КонецПроцедуры
	...
	Если  ЧтениеXML.ЛокальноеИмя=ИмяТэга  и ЧтениеXML.ТипУзла=ТипУзлаXML.НачалоЭлемента Тогда
		...
		Пока не (ЧтениеXML.ЛокальноеИмя=ИмяТэга  и ЧтениеXML.ТипУзла=ТипУзлаXML.КонецЭлемента) Цикл	
			...
			Если ТЗрезультат.Колонки.Найти(ЧтениеXML.ЛокальноеИмя)<>Неопределено  и ЧтениеXML.ТипУзла=ТипУзлаXML.НачалоЭлемента Тогда
				...
				ЧтениеXML.Прочитать();
				Если ЧтениеXML.ТипУзла=ТипУзлаXML.КонецЭлемента Тогда Продолжить; КонецЕсли;
				...
				Пока не (ЧтениеXML.ЛокальноеИмя=ИмяТэга  и ЧтениеXML.ТипУзла=ТипУзлаXML.КонецЭлемента) Цикл	
					...
					Если ТЗрезультат.Колонки.Найти(ЧтениеXML.ЛокальноеИмя)<>Неопределено  и ЧтениеXML.ТипУзла=ТипУзлаXML.НачалоЭлемента Тогда
						...
						Если ТЗрезультат.колонки[ИмяКолонки].ТипЗначения=новый ОписаниеТипов("ТаблицаЗначений") Тогда
							новаяСтрока[ИмяКолонки]=ОбработатьТэгВер2(ЧтениеXML,СтруктураФайла[ИмяКолонки].ИмяТэга,ИмяКолонки,СтруктураФайла[ИмяКолонки].тз);
						Иначе
							ЧтениеXML.Прочитать();
							Если ЧтениеXML.ТипУзла=ТипУзлаXML.Текст Тогда
								новаяСтрока[ИмяКолонки]=ЧтениеXML.Значение;
...
Функция ОбработатьЗагруженныеССайтаДанные(ФайлЗагрузки,ИзСтоки=Ложь)   Экспорт
	...
	тзДоставленныеТранзиты	=	ТЗпоСтруктуре(новый Структура("orderType,pid,uid,date,arrivalNumber,code,send,result,groupCode","Строка","Число","Число","Дата","Строка","Строка","Число","Строка","Строка"));
	тзУчастники				=	ТЗпоСтруктуре(новый Структура("id,name,status,phone,modified,rating,result,message,send,city_id","Число","Строка","Число","Строка","Дата","Число","Строка","Строка","Число","Строка"));
	тзЗаказы				=	ТЗпоСтруктуре(новый Структура("id,orgid,result,message,modified,send,secureCode","Строка","Число","Строка","Строка","Дата","Число","Строка"));
	тзПокупки               =	ТЗпоСтруктуре(новый Структура("id,name,orgid,result,message,modified,send,secureCode,city_id","Число","Строка","Число","Строка","Строка","Дата","Число","Строка","Строка"));
	тзКарты                 =	ТЗпоСтруктуре(новый Структура("id,uid,status,modified,result,message,send","Число","Число","Строка","Дата","Строка","Строка","Число"));
	тзКоробки				=	ТЗпоСтруктуре(новый Структура("id, packageId, pid, orderId, uid, type, result, message,modified,send","Строка","Число","Число","Строка","Число","Строка","Строка","Строка","Дата","Число"));
	тзТочки					=	ТЗпоСтруктуре(новый Структура("code,name,city,address,unique_id, canProcessGroup,result,city_id","Строка","Строка","Строка","Строка","Строка","Число","Строка","Строка"));
	тзГруппы				=	ТЗпоСтруктуре(новый Структура("distributorCode,uniqueKey,name,items,result","Строка","Строка","Число","Строка"));
	тзСДЭК					=	ТЗпоСтруктуре(новый Структура("code,name,city,address,unique_id, canProcessGroup,result","Строка","Строка","Строка","Строка","Строка","Число","Строка"));
	тзПостомат				=	ТЗпоСтруктуре(новый Структура("code,name,city,address,unique_id, canProcessGroup,result","Строка","Строка","Строка","Строка","Строка","Число","Строка"));
	тзГорода				=	ТЗпоСтруктуре(новый Структура("id,region_id,region,name,send,result","Строка","Строка","Строка","Строка","Строка","Строка"));
	Ошибки=Неопределено;
...
Процедура ОбработатьОшибочныеЗаписи(ТЗ,НаименованиеСправочника,Ошибки)
	...
	Если СтрокиСОшибками.Количество()>0 Тогда
		Для каждого стр из СтрокиСОшибками Цикл 
			ТекстДляОднойОшибки=НаименованиеСправочника+":для кода """+стр.send+""" получен ответ с сайта """+стр.message+"""";
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,,ТекстДляОднойОшибки,,,);
	        ТЗ.Удалить(стр);
		КонецЦикла	
	КонецЕсли
...
Функция ДобавитьВыбратьГородСП(КодГорода,СсылкаГород=неопределено)
	Если ЗначениеЗаполнено(СсылкаГород) Тогда
		Город=СсылкаГород;
		
	ИначеЕсли СокрЛП(КодГорода)="" Тогда
		Город=Справочники.ГородаСП.ПустаяСсылка();
	Иначе
		Город=Справочники.ГородаСП.НайтиПоКоду(КодГорода);
		Если не ЗначениеЗаполнено(Город) Тогда
			обГород=Справочники.ГородаСП.СоздатьЭлемент();
			обГород.Код=КодГорода;
			обГород.Догрузить=Истина;
			обГород.Наименование="город с кодом "+КодГорода;
			Попытка
			  обГород.Записать();
			  Город=обГород.Ссылка;
			Исключение
			  Город=Справочники.ГородаСП.ПустаяСсылка();
			КонецПопытки;
		КонецЕсли;
...
Функция 	ОбработатьОтветыНаСменуСтатусовГД(ПолученныеДанные) экспорт
	СтруктураФайлаПродление=новый Структура("delivery_group_id,result,message,Ссылка","Строка","Строка","Строка","СправочникСсылка.Коробки");
	СтруктураФайлаИсключение=новый Структура("delivery_group_id,result,message,Ссылка,Покупка,Участник","Строка","Строка","Строка","СправочникСсылка.Коробки","СправочникСсылка.Покупки","СправочникСсылка.Участники");
	СтруктураФайлаУдаление  =новый Структура("delivery_group_id,result,message,Ссылка,Покупка,Участник","Строка","Строка","Строка","СправочникСсылка.Коробки","СправочникСсылка.Покупки","СправочникСсылка.Участники");
	СтруктураФайлаВзвешивание=новый Структура("delivery_group_id,result,message,Ссылка","Строка","Строка","Строка","СправочникСсылка.Коробки");
	...
	КонецЦикла;
	Результат=Новый Структура("Продление,Взвешивание,Исключение,Удаление",тзГруппПродление, тзГруппВзвешивание,тзГруппИсключение,тзГруппУдаление);
	
	Возврат Результат;
КонецФункции

Процедура 	ЗаполнитьСсылкиВТЗ(ТЗ,Код,Ссылка,Справочник)
	Если  ТЗ					<> Неопределено и
		  ТЗ.Колонки.Найти(Код) <> Неопределено  Тогда
...
Функция ОбработатьСозданныеНаСайтеКоробки(ПолученныеДанные) экспорт
	...
	СтруктураФайла	= новый Структура("uniqueKey,barcode,distributorName,ownerName,ordersNumber,code,createdAt,name,items,message,result",
											"Строка","Строка","Строка","Строка","Строка","Строка","Строка","Строка",СтруктураItems,"Строка","Строка");
		...
		Если стр.result="error" Тогда
			...
			Если стр.items.Количество()>0 Тогда
				Для каждого итем из стр.items Цикл
					ТекстОшибки="ошибка: "+ итем.message+":"+Символы.ПС+
									"	orderType: "+ итем.orderType+","+Символы.ПС+
									"	orderId:   "+ итем.orderId+","+Символы.ПС+
									"	pid:       "+ итем.pid+","+Символы.ПС+
									"	uid:       "+ итем.uid+";"+Символы.ПС;
					ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,"",ТекстОшибки,);
				КонецЦикла;	
			КонецЕсли;
...
Функция ОбработатьАдресаЗаказовССайта(ПолученныеДанные) экспорт
	...
	СтруктураФайла=новый Структура("orderType,uid,pid,orderId,type,recipient_name,zipcode,region,city,address,address_street,address_building,address_apartment,phone,email,pvz_code,city_code ,message,result",
											"Строка","Строка","Строка","Строка","Строка","Строка","Строка","Строка","Строка","Строка","Строка","Строка","Строка","Строка","Строка","Строка","Строка","Строка","Строка");
...
Функция  superGroupsXDTOвТЗ(ЧтениеXML,Регистрация)           //Загрузка приема
	...
	Структураincomes=новый Структура("documentNumber,processedAt,actualDeliveryDate,message,result",
	"Строка","Дата","Дата","Строка","Строка");
	ТЗincomes=ТЗпоСтруктуре(Структураincomes);											
...
Функция  superGroupStagesXDTOвТЗ(ЧтениеXML)    //Загрузка приема
	...
	Структураincomes=новый Структура("documentNumber,message,result",
...
//	ПолученныеДанные=СтрЗаменить(ПолученныеДанные,"</distributors","</distributors1");
	...
	Если Выборка.result="error" Тогда Возврат Справочники.Пристрой.ПустаяСсылка(); КонецЕсли;
