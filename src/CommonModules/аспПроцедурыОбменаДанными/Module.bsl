...
Процедура ЗаполнениеКонстант() Экспорт
	...
	Если Не ЗначениеЗаполнено(ПараметрОбмена.ДатаЗагрузки) Тогда
		обПараметрыЗагрузки 				= ПараметрОбмена.ПолучитьОбъект();
		обПараметрыЗагрузки.ДатаЗагрузки 	= Константы.ПоследняяЗагрузкаГруппОбъединеннойДоставки.Получить();
		обПараметрыЗагрузки.Записать();
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(ПараметрОбмена.ДатаЗагрузки) Тогда
		обПараметрыЗагрузки 				= ПараметрОбмена.ПолучитьОбъект();
		обПараметрыЗагрузки.ДатаЗагрузки 	= Константы.ПоследняяЗагрузкаКарты.Получить();
		обПараметрыЗагрузки.Записать();
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(ПараметрОбмена.ДатаЗагрузки) Тогда
		обПараметрыЗагрузки 				= ПараметрОбмена.ПолучитьОбъект();
		обПараметрыЗагрузки.ДатаЗагрузки 	= Константы.ПоследняяЗагрузкаПокупок.Получить();
		обПараметрыЗагрузки.Записать();
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(ПараметрОбмена.ДатаЗагрузки) Тогда
		обПараметрыЗагрузки 				= ПараметрОбмена.ПолучитьОбъект();
		обПараметрыЗагрузки.ДатаЗагрузки 	= Константы.ПоследняяЗагрузкаПосылок.Получить();
		обПараметрыЗагрузки.Записать();
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(ПараметрОбмена.ДатаЗагрузки) Тогда
		обПараметрыЗагрузки 				= ПараметрОбмена.ПолучитьОбъект();
		обПараметрыЗагрузки.ДатаЗагрузки 	= Константы.ПоследняяЗагрузкаУчастников.Получить();
		обПараметрыЗагрузки.Записать();
	КонецЕсли;
...
Процедура ОбработатьТэг(ЧтениеXML, ИмяТэга, ТЗрезультат, СтруктураАтрибутов = Неопределено)
	...
	КонецЕсли;
КонецПроцедуры
Функция ОбработатьТэгВер2(ЧтениеXML, ИмяТэга, ИмяСписка, СтруктураФайла) Экспорт
...
Функция ОбработатьЗагруженныеССайтаДанные(ФайлЗагрузки, ИзСтоки = Ложь) Экспорт
	...
	тзУчастники				=	ТЗпоСтруктуре(
		Новый Структура("id,name,status,phone,modified,rating,result,message,send,city_id", "Число", "Строка", "Число",
		"Строка", "Дата", "Число", "Строка", "Строка", "Число", "Строка"));
...
Процедура ЗаполнитьСсылкиВТЗ(ТЗ, Код, Ссылка, Справочник) Экспорт
