...
Функция Получить_ПоДате(ДатаЗагрузки	= неопределено) Экспорт
	...
	Если результатРазбора.АвторизацияВыполнена Тогда
		...
		Обновить_ГруппыДоставки_ПоКодуДате(результатРазбора.deliveryGroups, хмл_отправили);
...
Процедура Обновить_ГруппыДоставки_ПоКодуДате(тзГрупп, Отправили = "") 
	Ошибки		= Неопределено;
	Для каждого стр из тзГрупп Цикл
		...
		Иначе	
			...
			Иначе	// Доставка Транспортной Компанией
				...
				ГруппаДоставки	= СП_РаботаСоСправочниками.ПолучитьКоробкуПо_Коду(КодКоробки);
				параметрыКоробки.Вставить("ШК", 					СП_Штрихкоды.ПолучитьМегаордер(ГруппаДоставки,параметрыКоробки.УчастникГД ,параметрыКоробки.ТочкаНазначения) );
				СП_РаботаСоСправочниками.ОбновитьКоробку(ГруппаДоставки, параметрыКоробки);
				стр.Ссылка = ГруппаДоставки; 
				...
				///
				НомерДокумента = стр.id;
				ссылка_Отпарвление = Документы.ОтправлениеТранзита.НайтиПоНомеру(НомерДокумента);
				Если не ЗначениеЗаполнено(ссылка_Отпарвление) Тогда
					
					объект_Отпавление 					= Документы.ОтправлениеТранзита.СоздатьДокумент();
					объект_Отпавление.Дата				= ТекущаяДата();
					объект_Отпавление.Номер 			= НомерДокумента;
					объект_Отпавление.Коробка			= ГруппаДоставки;
					СтрокаПротокола.Описание			= 	"Создание группы доставки";
				Иначе	
					объект_Отпавление = ссылка_Отпарвление.ПолучитьОбъект();
					СтрокаПротокола.Описание			= 	"Обновление группы доставки";					 
					
				КонецЕсли;
				объект_Отпавление.ОбновитьИЗаполнитьОстатками();
				объект_Отпавление.КоличествоВГруппе 	= объект_Отпавление.Заказы.Количество();
				объект_Отпавление.КоличествоНаСкладе 	= объект_Отпавление.КоличествоВГруппе - объект_Отпавление.Заказы.НайтиСтроки(Новый Структура("МестоХранения",Справочники.МестаХранения.ПустаяСсылка())).Количество();
				Попытка
					объект_Отпавление.Записать(РежимЗаписиДокумента.Проведение);
					
					
					СтрокаПротокола.Результат= Истина;
					ПолученныеДанные = "";
					Для каждого элем из параметрыКоробки Цикл					
						ПолученныеДанные = ПолученныеДанные + элем.Ключ + " - " + элем.Значение + Символы.ПС;
					КонецЦикла;
					ПолученныеДанные = ПолученныеДанные + СтоСПОбмен_Общий.ПолучитьСтрокуXML(стр.orders);
					СтрокаПротокола.ПолученныеДанные	=  ПолученныеДанные;
					СтрокаПротокола.ДатаОкончания			= ТекущаяДата();
					СтоСПОбмен_Выгрузка100сп.СохранитьПротоколОбмена(СтрокаПротокола, объект_Отпавление.Ссылка);
...
#Область УдалитьЗаказИзГруппыДоставки

// Параметры.ГруппаДоставки
// Параметры.Участник
// Параметры.КодЗаказа
// Параметры.СообщениеУдаления

Функция ПолучитьХМЛ_УдалитьЗаказИзГруппы(Параметры,Исключить) 
	 ...
	 ЗаписьXML.ЗаписатьАтрибут("xmlns:xsd","http://www.w3.org/2001/XMLSchema");	
		...
		ЗаписьXML.ЗаписатьНачалоЭлемента(?(Исключить,"groupsDetach","groupsUnlink")); /// Список

		ЗаписьXML.ЗаписатьНачалоЭлемента("group");
			 
			 ЗаписьXML.ЗаписатьНачалоЭлемента("delivery_group_id");
			 	ЗаписьXML.ЗаписатьТекст(Формат(Число(СтрЗаменить(Параметры.ГруппаДоставки.Код,"гд_","")),"ЧГ=0"));
			 ...
			 ЗаписьXML.ЗаписатьНачалоЭлемента("uid");
			 	ЗаписьXML.ЗаписатьТекст(Формат(Параметры.Участник.Код,"ЧГ=0"));
			 ЗаписьXML.ЗаписатьКонецЭлемента();
					 
			 ЗаписьXML.ЗаписатьНачалоЭлемента("message");
			 	ЗаписьXML.ЗаписатьТекст(Формат(Параметры.СообщениеУдаления,"ЧГ=0"));
			 ЗаписьXML.ЗаписатьКонецЭлемента();

			 ЗаписьXML.ЗаписатьНачалоЭлемента("orderId");
				 ЗаписьXML.ЗаписатьТекст(Формат(Параметры.КодЗаказа,"ЧГ=0"));
			 ЗаписьXML.ЗаписатьКонецЭлемента();
//					 Иначе	 
...
Функция Разбор_УдалитьЗаказИзГруппы(ТЗ_Ответ,ПолученныеДанные) 
	СтруктураФайлаУдаление  =	Новый Структура("delivery_group_id,	result,    message,	 Ссылка,					 Покупка,					 Участник", 
												"Строка",			"Строка",  "Строка", "СправочникСсылка.Коробки", "СправочникСсылка.Покупки", "СправочникСсылка.Участники");
	...
	Пока ЧтениеXML.Прочитать() Цикл
		...
		Если 	ЧтениеXML.ЛокальноеИмя = "groupsUnlink" И 
				ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
					
			ТЗ_Ответ	= аспПроцедурыОбменаДанными.ОбработатьТэгВер2(ЧтениеXML, "group", "groupsUnlink", СтруктураФайлаУдаление);
			аспПроцедурыОбменаДанными.ЗаполнитьСсылкиВТЗ(ТЗ_Ответ, "delivery_group_id", "Ссылка", "Коробки");
			аспПроцедурыОбменаДанными.ЗаполнитьСсылкиВТЗ(ТЗ_Ответ, "pid", "Покупка", "Покупки");
			аспПроцедурыОбменаДанными.ЗаполнитьСсылкиВТЗ(ТЗ_Ответ, "uid", "Участник", "Участники");
		ИначеЕсли 	ЧтениеXML.ЛокальноеИмя = "groupsDetach" И 
				ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
					
			ТЗ_Ответ	= аспПроцедурыОбменаДанными.ОбработатьТэгВер2(ЧтениеXML, "group", "groupsDetach", СтруктураФайлаУдаление);
			аспПроцедурыОбменаДанными.ЗаполнитьСсылкиВТЗ(ТЗ_Ответ, "delivery_group_id", "Ссылка", "Коробки");
			аспПроцедурыОбменаДанными.ЗаполнитьСсылкиВТЗ(ТЗ_Ответ, "pid", "Покупка", "Покупки");
			аспПроцедурыОбменаДанными.ЗаполнитьСсылкиВТЗ(ТЗ_Ответ, "uid", "Участник", "Участники");
...
Функция УдалитьЗаказИзГруппы(Параметры, Исключить = Ложь) Экспорт
	
	СтрокаПротокола						= 	СтоСПОбмен_Выгрузка100сп.СтруктураПротокола();

	СтрокаПротокола.ДатаНачала			=	ТекущаяДата();				
	СтрокаПротокола.Описание			= 	?(Исключить,"Исключение","Удаление")+" заказа из группы" ;
	

	код_группы		= Число(СтрЗаменить(Параметры.ГруппаДоставки.Код,"гд_",""));
	хмл_отправили	= ПолучитьХМЛ_УдалитьЗаказИзГруппы(Параметры,Исключить);
	...
	СтрокаПротокола.ДатаОкончания		= 	ТекущаяДата();
	СтоСПОбмен_Выгрузка100сп.СохранитьПротоколОбмена(СтрокаПротокола, Параметры.ГруппаДоставки.ОтправлениеТранзита);
	
			
	//Если хмл_получили= "Не удалось соеденится с сайтом" Тогда Возврат КонецЕсли;
	ТЗ_Ответ=Новый ТаблицаЗначений;
	Разбор_УдалитьЗаказИзГруппы(ТЗ_Ответ,хмл_получили);
	найденые_Строки=ТЗ_Ответ.НайтиСтроки(новый Структура("delivery_group_id",код_группы));
	Если найденые_Строки.Количество()=0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Для группы с кодом "+код_группы+" не получен ответ при установке метода оплаты!");	
		Возврат ложь;
	...
	КодыГрупп = новый СписокЗначений;
	КодыГрупп.Добавить(Параметры.ГруппаДоставки);
	Загрузить_ПоКодам(КодыГрупп);
	Возврат Истина;
КонецФункции	
