...
Функция Загрузить_ПоКодам(КодыГрупп) Экспорт
...
Процедура Обновить_ГруппыДоставки_ПоКодуДате(тзГрупп) 
	...
	Для каждого стр из тзГрупп Цикл
		...
		Иначе	
			...
			параметрыКоробки.Вставить("ТочкаНазначения",		?(стр.recipient_code="",Справочники.ТочкиРаздачи.ТочкаНеОпределена,СП_РаботаСоСправочниками.ПолучитьПунктВыдачиПо_Коду(стр.recipient_code)));
			...
			параметрыКоробки.Вставить("МетодОплаты",			?(стр.payment_method	= "",Перечисления.МетодыОплаты.ПустаяСсылка(),Перечисления.МетодыОплаты[стр.payment_method]));
			параметрыКоробки.Вставить("ДатаЗагрузки",			СтоСПОбмен_Общий.ДатаИзСтроки(стр.created_at));
	...
	Для каждого Заказ из Объект_distributors.deliveryGroupTransitCode.order Цикл
		...
		Для каждого стр из найденыСтроки Цикл
			...
			Если стр.result="ok" или стр.result="ap" Тогда
				стр.ТочкаНазначение  			= СП_РаботаСоСправочниками.ПолучитьПунктВыдачиПо_Коду(Заказ.distributor_code);
...
Функция Компановка_ЗапросТочкиСбора(ТЗЗаказов) 
	...
	отбор_ЗаказыВСвоихГруппах=новый Структура("ТочкаНазначение",Константы.СвояТочка.Получить());
