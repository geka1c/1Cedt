...
&НаКлиенте
Процедура ЗаменаНомераОтправленияТранзита(Команда)
	ЗаменаНомераОтправленияТранзитаНаСервере();
КонецПроцедуры

Процедура ЗаменаНомераОтправленияТранзитаНаСервере()
	...
	Запрос.Текст =
		...
		|ГДЕ
		|	НЕ ОтправлениеТранзита.ПометкаУдаления
		|	И ОтправлениеТранзита.Коробка.ВидСтикера = ЗНАЧЕНИЕ(Перечисление.ВидыСтикеров.ГруппаДоставки)";
	...
	Пока Выборка.Следующий() Цикл
		об 						= Выборка.ссылка.ПолучитьОбъект();
		об.Номер 				= Формат(Число(СтрЗаменить(Выборка.Ссылка.Коробка.Код,"гд_","")),"ЧГ=0;");
		об.Статус 				= Выборка.Ссылка.Коробка.СтатусГруппыДоставки;
		об.КоличествоВГруппе 	= об.Заказы.Количество();
		об.КоличествоНаСкладе 	= об.КоличествоВГруппе - об.Заказы.НайтиСтроки(Новый Структура("МестоХранения",Справочники.МестаХранения.ПустаяСсылка())).Количество();
		Попытка
			об.Записать()
		Исключение
...
&НаКлиенте
Процедура ЗаполнитьШКВГруппахДоставки(Команда)
	ЗаполнитьШКВГруппахДоставкиНаСервере();
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьШКВГруппахДоставкиНаСервере()
	...
	Запрос.Текст =
		...
		|ГДЕ
		//|	Коробки.ШК = ЗНАЧЕНИЕ(Справочник.Мегаордера.ПустаяСсылка)
	...
	Пока Выборка.Следующий() Цикл
		обГруппаДоставки = Выборка.Ссылка.ПолучитьОбъект();
		Попытка
			обГруппаДоставки.Записать();
		Исключение
//			ТесктОшибки = ОписаниеОшибки();
//			ОбщегоНазначенияКлиентСервер.С
		КонецПопытки;
	КонецЦикла;
КонецПроцедуры
