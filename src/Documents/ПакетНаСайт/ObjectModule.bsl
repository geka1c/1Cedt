...
Функция РазобратьРасходы()  
	...
	|	КОНЕЦ КАК pid,
	|	РасходнаяПокупки.ПокупкаСсылка.Владелец.Код КАК orgid,
...
Функция РазобратьТранзиты()  
	...
	|	И Выбор Когда ТИПЗНАЧЕНИЯ(ВыдачаТранзитаПокупки.Партия) = ТИП(Документ.курьерскаяДоставка)
	|	И ВыдачаТранзитаПокупки.Партия.СвойКурьер
	|;
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ПОДСТРОКА(ВыдачаТранзитаПокупки.Ссылка.Номер, 5, 9) КАК arrivalNumber,
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(ВыдачаТранзитаПокупки.Покупка) = ТИП(Справочник.Покупки)
	|			ТОГДА ""sp""
	|		КОГДА ТИПЗНАЧЕНИЯ(ВыдачаТранзитаПокупки.Покупка) = ТИП(Справочник.Заказы)
	...
	|	NULL,
	|	КурьерскаяДоставкаЗаказыНаСкладе.Посылка.Код КАК ПосылкаПокупкаКод1,
	|	NULL,
	|	NULL,
	|	КурьерскаяДоставкаЗаказыНаСкладе.СтоимостьХранения,
	|	КурьерскаяДоставкаЗаказыНаСкладе.Ссылка.Дата,
	|	ПОДСТРОКА(КурьерскаяДоставкаЗаказыНаСкладе.Партия.Номер, 5, 9) КАК Партия,
	|	""package"",
	|	""""
	|ИЗ
...
Процедура ДесериализоватьИзХМЛ() Экспорт
	...
	Если тз<>неопределено Тогда  
		Для каждого стр из тз Цикл
			...
			Детально[стр.НомерСтроки-1].ПерваяТочкаПриема	= СП_РаботаСоСправочниками.ПолучитьПунктВыдачиПо_Коду(Стр.firstIncomeCode);
