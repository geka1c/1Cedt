...
Процедура УдалитьЗаказИзГруппы(Команда)
	Если 	ЗначениеЗаполнено(Элементы.Заказы.ТекущиеДанные) и  
			ЗначениеЗаполнено(Объект.Коробка) И
			ЗначениеЗаполнено(Объект.Участник)   Тогда
		ПараметрыЗаказа = Новый Структура();
		ПараметрыЗаказа.Вставить("ГруппаДоставки",	Объект.Коробка);
		ПараметрыЗаказа.Вставить("Участник",		Объект.Участник);
		ПараметрыЗаказа.Вставить("КодЗаказа",		Элементы.Заказы.ТекущиеДанные.КодЗаказа);
	

		УдалитьЗаказИзГруппыНаСервере(ПараметрыЗаказа);
	Иначе
		ОбщегоНазначенияКлиентСервер.сообщитьПользователю("Поля: ""Группа доставки, Участник, Код заказа"" должны быть заполнены!");
	КонецЕсли;
КонецПроцедуры

&НаСервере
Процедура УдалитьЗаказИзГруппыНаСервере(ПараметрыЗаказа)
	СтоСПОбмен_ГруппыДоставки.УдалитьЗаказИзГруппы(ПараметрыЗаказа);
КонецПроцедуры

&НаКлиенте
Процедура ИсключитьЗаказИзГруппы(Команда)
	Если 	ЗначениеЗаполнено(Элементы.Заказы.ТекущиеДанные) и  
			ЗначениеЗаполнено(Объект.Коробка) И
			ЗначениеЗаполнено(Объект.Участник)   Тогда
		ПараметрыЗаказа = Новый Структура();
		...
		ИсключитьЗаказИзГруппыНаСервере(ПараметрыЗаказа);
	Иначе
		ОбщегоНазначенияКлиентСервер.сообщитьПользователю("Поля: ""Группа доставки, Участник, Код заказа"" должны быть заполнены!");
	КонецЕсли;
...
&НаСервере
Процедура ИсключитьЗаказИзГруппыНаСервере(ПараметрыЗаказа)
	СтоСПОбмен_ГруппыДоставки.УдалитьЗаказИзГруппы(ПараметрыЗаказа,Истина);
КонецПроцедуры
