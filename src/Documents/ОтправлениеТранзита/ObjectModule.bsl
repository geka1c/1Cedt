...
Функция УдалитьЗаказИзГруппы_Api()
	
КонецФункции	


Функция ИсключитьЗаказИзГруппы_Api()
...
Процедура РасчитатьСостояние() Экспорт
	
	Взвешено = (СтоимостьДоставки>0 или СтоимостьДоставкиПредоплата>0);
	
КонецПроцедуры	

Функция ИзменилсяСоставГруппы() Экспорт
	СоставИзменился = Ложь;
		ТаблицаГруппы =  Коробка.Состав.Выгрузить();
		ТаблицаГруппы.Свернуть("Таблица, Цена, КодЗаказа");
		ТаблицаДокумента = Заказы.Выгрузить();
		ТаблицаДокумента.Свернуть("Покупка,КодЗаказа,ОбъявленнаяСтоимость");
		Для Каждого Элем из ТаблицаГруппы Цикл 
			масСтрок = ТаблицаДокумента.НайтиСтроки(Новый Структура("Покупка,КодЗаказа,ОбъявленнаяСтоимость",
														элем.Покупка, элем.КодЗаказа, элем.Цена	));
			Если масСтрок.Количество()<>1 Тогда
				СоставИзменился = Истина;
				Прервать;
			Иначе 
				ТаблицаДокумента.Удалить(масСтрок[0]);	
			КонецЕсли;																	
		КонецЦикла;
...
Функция ОбновитьИЗаполнитьОстатками() Экспорт



	///Обновляем
//		списокКодов = Новый СписокЗначений;
//		списокКодов.Добавить(Формат(число(СтрЗаменить(Коробка.Код, "гд_", "")), "ЧГ=0;"));
//		СтоСПОбмен_ГруппыДоставки.Загрузить_ПоКодам(списокКодов);
	...
	ТочкаОтправитель 	= Константы.СвояТочка.Получить();
	ТочкаНазначения 	= Коробка.ТочкаНазначения;
	Участник			= Коробка.УчастникГД;
	Статус				= Коробка.СтатусГруппыДоставки;
	
	
	таблицаЗаказов = Коробка.Состав.Выгрузить(новый Структура("Исключить, Удалить",Ложь,ложь),"Покупка, Участник, КодЗаказа, Цена");
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ЗаказыВПосылкахОстатки.Покупка,
		|	ЗаказыВПосылкахОстатки.Участник,
		|	ЗаказыВПосылкахОстатки.КодЗаказа,
		|	ЗаказыВПосылкахОстатки.Посылка,
		|	ЗаказыВПосылкахОстатки.Партия,
		|	ЗаказыВПосылкахОстатки.КоличествоОстаток КАК Количество
		|ПОМЕСТИТЬ втРНЗаказыВПосылках
		|ИЗ
		|	РегистрНакопления.ЗаказыВПосылках.Остатки(, (Покупка, Участник, КодЗаказа) В
		|		(ВЫБРАТЬ
		...
		|
		|ВЫБРАТЬ
		|	ЗаказыВПосылках.Покупка,
		|	ЗаказыВПосылках.Участник,
		|	ЗаказыВПосылках.КодЗаказа,
		|	ЗаказыВПосылках.Посылка,
		|	ЗаказыВПосылках.Партия,
		|	ЗаказыВПосылках.Количество
		|ИЗ
		|	РегистрНакопления.ЗаказыВПосылках КАК ЗаказыВПосылках
		|ГДЕ
		|	ЗаказыВПосылках.Регистратор = &Регистратор
		|	И (ЗаказыВПосылках.Покупка, ЗаказыВПосылках.Участник, ЗаказыВПосылках.КодЗаказа) В
		|		(ВЫБРАТЬ
		|			тзВнешн.Покупка КАК Покупка,
		|			тзВнешн.Участник КАК Участник,
		|			тзВнешн.КодЗаказа КАК КодЗаказа
		...
		|	втРНЗаказыВПосылках.Посылка КАК Посылка
		|ПОМЕСТИТЬ заказыСпосылками
		|ИЗ
		|	тзВнешн КАК тзВнешн
		|		ЛЕВОЕ СОЕДИНЕНИЕ втРНЗаказыВПосылках КАК втРНЗаказыВПосылках
		|		ПО тзВнешн.Покупка = втРНЗаказыВПосылках.Покупка
		|		И тзВнешн.Участник = втРНЗаказыВПосылках.Участник
		|		И тзВнешн.КодЗаказа = втРНЗаказыВПосылках.КодЗаказа
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|ГДЕ
		|	Транзит.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
		|	И Транзит.Регистратор = &Регистратор
		|	И ((Транзит.ПокупкаСсылка, Транзит.Участник) В
		|		(ВЫБРАТЬ
		|			заказыСпосылками.Покупка КАК Покупка,
		|			заказыСпосылками.Участник КАК Участник
		|		ИЗ
		|			заказыСпосылками КАК заказыСпосылками)
		|	ИЛИ (Транзит.ПокупкаСсылка, Транзит.Участник) В
		|		(ВЫБРАТЬ
		|			заказыСпосылками.Посылка КАК Покупка,
		|			заказыСпосылками.Участник КАК Участник
		|		ИЗ
...
Процедура ПересчитатьСтоимостиОтправки() Экспорт
	
	Если ТарифТК = 0 Тогда Возврат; КонецЕсли;
		
	Если Коробка.ВидСтикера	= Перечисления.ВидыСтикеров.ЗаказТК или не ЗначениеЗаполнено(Коробка) Тогда
		ИтогоСтоимость		= ТарифТК + ТарифПВ;	
	ИначеЕсли Коробка.ВидСтикера	= Перечисления.ВидыСтикеров.ГруппаДоставки Тогда
		...
		ИначеЕсли Коробка.МетодОплаты	=	Перечисления.МетодыОплаты.cod Тогда
			ТарифыСОбъявленнойСтоимостью 	= ТарифТК+ТарифПВнп+ОбъявленнаяСтоимость*ПроцентОС/100;	
			ИтогоСтоимость					= Окр(ТарифыСОбъявленнойСтоимостью+ТарифыСОбъявленнойСтоимостью*Процент/100,0);
		Иначе
			ТарифыСОбъявленнойСтоимостью 	= ТарифТК+ТарифПВнп+ОбъявленнаяСтоимость*ПроцентОС/100;
			СтоимостьДоставки			= Окр(ТарифыСОбъявленнойСтоимостью + ТарифыСОбъявленнойСтоимостью*Процент/100,0);
			СтоимостьДоставкиПредоплата	= Окр(ТарифыСОбъявленнойСтоимостью,0);
			ИтогоСтоимость=0;
		КонецЕсли;
	КонецЕсли		
