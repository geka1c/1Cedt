...
Функция ТекстЗапроса_ГруппыДляПродления(ТекстыЗапроса)
	...
	ТекстЗапроса = "ВЫБРАТЬ
					...
					|	Справочник.Коробки КАК Коробки
					|ГДЕ
					|	Коробки.ДатаПервогоПоступления > ДАТАВРЕМЯ(1, 1, 1)  И
					|	 РАЗНОСТЬДАТ(Коробки.ДатаПервогоПоступления, &ТекущаяДата, ДЕНЬ) > Коробки.СрокХраненияГруппы  И
					|	 Коробки.Количество > Коробки.КоличествоНаСкладе
...
Функция ТекстЗапроса_ГруппыДляЗапросаОплаты(ТекстыЗапроса)
	...
	ТекстЗапроса = "ВЫБРАТЬ
	               ...
	               |ГДЕ
	               |	Коробки.Взвешено  и
				   |	Коробки.СтатусГруппыДоставки = ЗНАЧЕНИЕ(Перечисление.СтатусыГруппыДоставки.waitForOrders)
	               |	И Коробки.ВидСтикера = ЗНАЧЕНИЕ(Перечисление.ВидыСтикеров.ГруппаДоставки)";
