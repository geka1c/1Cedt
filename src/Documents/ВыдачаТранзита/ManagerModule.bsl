...
Функция ПечатьСупперГруппы(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	|	ВыдачаТранзита.ТочкаОтправитель КАК ТочкаОтправления,
	...
	|		Покупка.Владелец КАК Организатор,
	|		ВЫБОР
	|			КОГДА ТИПЗНАЧЕНИЯ(ВыдачаТранзита.Покупки.Покупка) = ТИП(Справочник.Коробки)
	|				ТОГДА ВыдачаТранзита.Покупки.Покупка.ВидСтикера
	|			ИНАЧЕ ТИПЗНАЧЕНИЯ(ВыдачаТранзита.Покупки.Покупка)
	|		КОНЕЦ КАК ТипПокупки,
	|		ВЫБОР
	|			КОГДА ТИПЗНАЧЕНИЯ(ВыдачаТранзита.Покупки.Покупка) = ТИП(Справочник.Коробки)
	|				и ВыдачаТранзита.Покупки.Покупка.ВидСтикера=Значение(Перечисление.ВидыСтикеров.ГС)
	|				ТОГДА Покупка.Код
	|			ИНАЧЕ Покупка.НомерЗаказа
	|		КОНЕЦ КАК КодПокупки,
	|		Участник.Код КАК КодУчастника
	|	) КАК Покупки,
	|	ВыдачаТранзита.Супергруппа КАК ШК,
...
Функция ПечатьДоставкаКурьером(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Пока ВыборкаПокупки.Следующий() Цикл
			...
			ОбластьПокупки.Параметры.Ник				= ВыборкаПокупки.Покупка.ОтправлениеТранзита.Участник;
			ОбластьПокупки.Параметры.Телефон			= ВыборкаПокупки.Покупка.ОтправлениеТранзита.Телефон;
...
Функция Получить_ТекстЗапроса_УпакованныеЗаказы(Запрос, ТекстыЗапроса, Регистры) Экспорт
	...
	Если ТипЗнч(Запрос.Параметры.Ссылка)=Тип("ДокументСсылка.ВыдачаТранзита") Тогда
		...
		|	ВыдачаТранзитаПокупки.Габарит КАК Габарит,
		|	ВыдачаТранзитаПокупки.Участник КАК Участник,
		|	ВыдачаТранзитаПокупки.Ссылка.ТочкаНазначения КАК Точка,
		|	ВыдачаТранзитаПокупки.Партия КАК Партия,
		|	1 КАК Количество
		|ИЗ
		|	Документ.ВыдачаТранзита.Покупки КАК ВыдачаТранзитаПокупки
...
Функция Получить_ТекстЗапроса_Расход(Запрос, ТекстыЗапроса, Регистры) Экспорт
	...
	Если ТипЗнч(Запрос.Параметры.Ссылка)=Тип("ДокументСсылка.ВыдачаТранзита") Тогда
		ТекстЗапроса = "ВЫБРАТЬ
		               ...
		               |	ВыдачаТранзитаПокупки.СтоимостьХранения КАК Сумма
		               |ИЗ
		               |	Документ.ВыдачаТранзита.Покупки КАК ВыдачаТранзитаПокупки
		               |ГДЕ
...
Функция ПолучитьТэг_transfer(Ссылка,НомерСтроки=Неопределено) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ЭтоЗаказТК Тогда
			Если ЗначениеЗаполнено(Выборка.КодЗаказа) Тогда
				СтоСП.Вставить_Тэг(ЗаписьXML,"orderId"	, 	Выборка.КодЗаказа);
...
Процедура ПроверитьНаличие(Объект,Отказ)      Экспорт
	...
	Запрос.Текст = 
		...
		|	СОстатками.Участник,
		|	СОстатками.Точка,
		|	СОстатками.КоличествоНаСкладе + СОстатками.КоличествоУпакованно как КоличествоНаОстатках,
		|	СОстатками.НомерСтроки КАК НомерСтроки
		|ИЗ
		|	СОстатками КАК СОстатками
		|ГДЕ
		|	СОстатками.Количество > (СОстатками.КоличествоНаСкладе + КоличествоУпакованно)
		|
		|УПОРЯДОЧИТЬ ПО
		|	НомерСтроки ";
