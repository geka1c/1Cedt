...
Функция ПроверитьСтатусыЗаказовНаСервере()
	...
	Запрос.Текст =
		...
		|	Выразить("""" как Строка(20)) КАК СтатусЗаказа,
	...
	Если Объект.ТочкаНазначения = Справочники.ТочкиРаздачи.НайтиПоКоду("0083") Тогда //СДЭК
		результат 		= Интеграция_СДЭК.СтатусыЗаказов(массивЗаказов);
		Если результат	= Неопределено Тогда Возврат Неопределено; КонецЕсли;
		Для каждого элем из результат.StatusReport.Order Цикл
			НомерЗаказа		=	элем.DispatchNumber;
			НомерПартии		=	элем.Number;	
			Получатель		=	элем.RecipientName;
			СтатусЗаказа	=	элем.Status.Description;
			ДатаСтатуса		=	элем.Status.Date;
			
			найденныеСтроки	=	тзСтатусов.НайтиСтроки(новый Структура("НомерЗаказа",НомерЗаказа));
			Для каждого стр из найденныеСтроки Цикл
				стр.статусЗаказа	=	СтатусЗаказа;
				стр.ДатаСтатуса		=	ДатаСтатуса;
				стр.Получатель		=	Получатель;
			КонецЦикла;
		КонецЦикла;
	КонецЕсли;
